...
def _build_cover_page(
    ...
    # --- Model info box ---
    meta_ax = fig.add_axes([0.07, 0.54, 0.86, 0.26])
    meta_ax.axis("off")
    ...
    # --- Hyperparameters box ---
    hyp_ax = fig.add_axes([0.07, 0.30, 0.86, 0.21])
    hyp_ax.axis("off")
    ...
    # Two-column layout for hyperparams
    ...
    # --- What is XGBoost? box ---
    def_ax = fig.add_axes([0.07, 0.07, 0.86, 0.20])
    def_ax.axis("off")
    def_ax.add_patch(
        plt.Rectangle((0, 0), 1, 1, fill=True, facecolor="#f0f7ff", edgecolor=REPORT_BORDER, linewidth=1.2)
    )
    def_ax.text(0.03, 0.88, "What is XGBoost?", fontsize=11, fontweight="bold", color=REPORT_ACCENT, va="top")
