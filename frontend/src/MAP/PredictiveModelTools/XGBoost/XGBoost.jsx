...
export default function XGBoost({ onClose, onShowMap }) {
  ...
  const [selectedSchema, setSelectedSchema] = useState(null); // 👈 NEW
  ...
  const normalizeDownloadUrl = (path) => {
    ...
    // fallback: use XGB download
    return `${API}/xgb/download?file=${enc}`;
  };
  ...
  const resetForNewSource = () => {
    ...
    setPreviewErr("");
    setPage(1);
    setExcludedIndices([]); // 👈 NEW
  };

  // ---------- Attribute preview helpers ----------
    ...
    try {
      ...
      const res = await fetch(
        `${API}/xgb/db-preview?schema=${encodeURIComponent(
          schema
        )}&table=${encodeURIComponent(tableName)}&limit=${PAGE_SIZE}&offset=${offset}`
      );
  ...
  const loadFilePreview = async (fileList, pageNum = 1) => {
    try {
      ...
      const res = await fetch(`${API}/xgb/preview`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const fetchDbFields = async (tableName) => {
    try {
      ...
      if (res.ok && data.fields) {
        ...
        setFiles([]);
        await loadDbPreview(schema, table, 1); // 👈 this calls /xgb/db-preview
      } else {
  ...
  const handleTrainModel = async () => {
    ...
    try {
      ...
      fd.append("scaler_choice", scaler || "None");
      ...
      if (!res.ok) {
        console.error("XGB train error:", data);
        alert(`Error: ${data.error || res.statusText}`);
      } else {
        const inter = data.interactive_data || {};
        const normalized = {
          ...data,
          residuals: inter.residuals || [],
          residual_bins: inter.residual_bins || [],
          residual_counts: inter.residual_counts || [],
          y_test: inter.y_test || [],
          preds: inter.preds || [],
        };

        if (normalized.downloads?.cama_csv) {
          previewCAMA(normalizeDownloadUrl(normalized.downloads.cama_csv));
        }
  ...
  const handleRunModel = async () => {
    ...
    const usingDatabase = !!selectedRunDbTable && !!selectedRunSchema;
    ...
    try {
      ...
      if (usingDatabase) {
        fd.append("schema", selectedRunSchema); // 👈 NEW
      ...
      if (!res.ok) {
        console.error("XGB run error:", data);
        alert(`Error: ${data.error || res.statusText}`);
      } else {
        // Add geojson URL if missing
        if (data.downloads?.shapefile && !data.downloads.geojson) {
          const enc = encodeURIComponent(data.downloads.shapefile);
          data.downloads.geojson = `${API}/xgb/preview-geojson?file=${enc}`;
        }
        if (data.downloads?.cama_csv) {
          previewCAMA(data.downloads.cama_csv);
        }
        setResult(data);
        setActiveTab("metrics");
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <label className="block text-sm font-medium mt-3 text-gray-300">
                  Scaler
                </label>
                <select
                  value={scaler}
        ...
        <button
          onClick={async () => {
            ...
            if (!geojsonLink && result.downloads.shapefile) {
              ...
              geojsonLink = `${API}/xgb/preview-geojson?file=${enc}`;
            }
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              const res = await fetch(`${API}/xgb/save-to-db`, {
                method: "POST",
        ...
        >
          ...
          >
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => document.getElementById("xgbRunInput").click()}
                  className="bg-[#F7C800] hover:bg-[#e6b800] text-black px-4 py-2 rounded-md"
                >
                  📂 Local
                </button>
                  ...
                  <li key={t}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedRunSchema(schema); // 👈 ADD
