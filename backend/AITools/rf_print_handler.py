...
rf_print_handler.py
-------------------
PDF report generation handler for the Random Forest training module.
Same design system as lr_print_handler.py and xgb_print_handler.py —
shared constants, layout, typography, and helper functions — but content
is Random Forest-specific: model hyperparameters on the cover, feature
importances (not coefficients), no t-test pages, and an RF-specific
final summary.
"""
...
# ---------------------------------------------------------------------------
# Report color constants  (identical across all handlers)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Low-level drawing helpers  (identical across all handlers)
# ---------------------------------------------------------------------------
...
def _build_cover_page(
    ...
    hyp_items = [
        ("n_estimators",  str(hyperparams.get("n_estimators", 100))),
        ("n_jobs",        str(hyperparams.get("n_jobs", -1))),
        ("random_state",  str(hyperparams.get("random_state", 42))),
        ("max_depth",     str(hyperparams.get("max_depth", "None (unlimited)"))),
        ("max_features",  str(hyperparams.get("max_features", "1.0"))),
        ("test_size",     str(hyperparams.get("test_size", "30%"))),
    ]
...
def _build_metrics_table_page(
    ...
    _text_box(fig, 0.10, 0.60, 0.80, notes_text,
...
def _build_feature_importance_page(
    ...
    fig = _new_page()
    _add_page_header(fig, "Feature Importance", "Mean decrease in impurity across all trees")
    ...
    chart_ax.set_title("Feature Importance (Mean Decrease in Impurity)", fontsize=11, fontweight="bold", color=REPORT_ACCENT, pad=10)
...
def _build_residual_distribution_page(
    ...
    # Linear y-axis ticks: 0, 2k, 4k, 6k, 8k, 10k
...
def _build_final_summary_page(
    ...
    notes = [
        f"This Random Forest model was trained using {len(feature_names)} predictor(s) with "
        f"{hyperparams.get('n_estimators', 100)} decision trees.",
        f"Overall fit should be judged through R² and diagnostic plots. Current R² is {metrics['R²']:.4f}.",
        "Feature importance scores reflect mean decrease in impurity and are not directly comparable to regression coefficients.",
...
def export_rf_report_and_artifacts(
    ...
    hyperparams = {
        ...
        "max_features": getattr(model, "max_features", "1.0"),
        "test_size":    "30%",
    }
